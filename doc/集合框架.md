...
- [2. 设计模式](#2-设计模式)
    ...
    - [2.2. 适配器模式](#22-适配器模式)
- [3. 主要类具体分析](#3-主要类具体分析)
    - [3.1. 基本类型不能做为 HashMap 的键值](#31-基本类型不能做为-hashmap-的键值)
    - [3.2. ArrayList 和 Vector 的联系和区别](#32-arraylist-和-vector-的联系和区别)
    - [3.3. 4.HashMap详解](#33-4hashmap详解)
        - [3.3.1. 内部数据结构](#331-内部数据结构)
        - [3.3.2. 插入原理](#332-插入原理)
        - [3.3.3. HashMap初始容量](#333-hashmap初始容量)
        - [3.3.4. HashMap的hash函数如何设计](#334-hashmap的hash函数如何设计)
        - [3.3.5. 为什么这样做异或操作](#335-为什么这样做异或操作)
        - [3.3.6. 为什么采用 hashcode 的高16位和低16位异或能降低 hash 碰撞？hash 函数能不能直接用 key 的 hashcode？](#336-为什么采用-hashcode-的高16位和低16位异或能降低-hash-碰撞hash-函数能不能直接用-key-的-hashcode)
        - [3.3.7. 1.8版本除了 hash 函数做优化，还有其他哪些优化](#337-18版本除了-hash-函数做优化还有其他哪些优化)
        - [3.3.8. 1.8版本做的优化的原因](#338-18版本做的优化的原因)
        - [3.3.9. HashMap是线程安全的吗？](#339-hashmap是线程安全的吗)
        - [3.3.10. 解决HashMap是线程不安全问题](#3310-解决hashmap是线程不安全问题)
        - [3.3.11. ConcurrentHashMap 的分段锁的实现原理](#3311-concurrenthashmap-的分段锁的实现原理)
        - [3.3.12. 链表转红黑树是链表长度达到阈值，这个阈值是多少？](#3312-链表转红黑树是链表长度达到阈值这个阈值是多少)
...
- Map 接口实现：
	...
	- ConcurrentHashMap：1.7 和 1.8 的实现不同
        ...
        - 1.8 移除 Segment，使锁粒度更小，Synchronized + CAS + Node + Unsafe 实现
	- Properties
- List接口实现：
...
#### 3.3.7. 1.8版本除了 hash 函数做优化，还有其他哪些优化
