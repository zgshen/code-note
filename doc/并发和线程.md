...
1. [1. 基本概念](#1-基本概念)
    1. [1.1. 进程、线程和协程](#11-进程线程和协程)
    2. [1.2. 并行与并发](#12-并行与并发)
    3. [1.3. 并发三特性和先行发生原则](#13-并发三特性和先行发生原则)
        1. [1.3.1. 原子性](#131-原子性)
        2. [1.3.2. 可见性](#132-可见性)
        3. [1.3.3. 有序性](#133-有序性)
        4. [1.3.4. 先行发生原则（happens-before）](#134-先行发生原则happens-before)
    4. [1.4. qps 和 tps 区别](#14-qps-和-tps-区别)
    5. [1.5. 多线程什么情况下使用](#15-多线程什么情况下使用)
2. [2. 线程](#2-线程)
    1. [2.1. 线程的生命周期](#21-线程的生命周期)
    2. [2.2. 创建线程](#22-创建线程)
3. [3. 线程关键字和方法函数相关](#3-线程关键字和方法函数相关)
    1. [3.1. wait notify 机制](#31-wait-notify-机制)
    2. [3.2. sleep 和 wait 区别](#32-sleep-和-wait-区别)
    3. [3.3. volatile 和 synchronized 区别](#33-volatile-和-synchronized-区别)
    4. [3.4. synchronized 和 Lock](#34-synchronized-和-lock)
    5. [3.5. yield](#35-yield)
    6. [3.6. join](#36-join)
    7. [3.7. Lock 接口六个方法](#37-lock-接口六个方法)
    8. [3.8. ReentrantLock 可重入锁](#38-reentrantlock-可重入锁)
    9. [3.9. 异步处理框架](#39-异步处理框架)
    10. [3.10. CAS](#310-cas)
    11. [3.11. volatile](#311-volatile)
    12. [3.12. ThreadLocal](#312-threadlocal)
4. [4. 线程池](#4-线程池)
    1. [4.1. JDK 线程池](#41-jdk-线程池)
        1. [4.1.1. Executors](#411-executors)
        2. [4.1.2. ThreadPoolExecutor](#412-threadpoolexecutor)
    2. [4.2. ThreadPoolExecutor 线程池参数](#42-threadpoolexecutor-线程池参数)
    3. [4.3. 线程池工作原理](#43-线程池工作原理)
5. [5. 线程安全](#5-线程安全)
    1. [5.1. 不可变](#51-不可变)
    2. [5.2. 互斥同步](#52-互斥同步)
    3. [5.3. 非阻塞同步](#53-非阻塞同步)
    4. [5.4. 无同步方案](#54-无同步方案)
        1. [5.4.1. 栈封闭](#541-栈封闭)
        2. [5.4.2. 线程本地存储（Thread Local Storage）](#542-线程本地存储thread-local-storage)
        3. [5.4.3. 可重入代码（Reentrant Code）](#543-可重入代码reentrant-code)
6. [6. 各种锁类型](#6-各种锁类型)
    1. [6.1. 互斥访问](#61-互斥访问)
    2. [6.2. 悲观锁和乐观锁](#62-悲观锁和乐观锁)
    3. [6.3. 自旋锁和适应性自旋锁](#63-自旋锁和适应性自旋锁)
    4. [6.4. 无锁、偏向锁、轻量级锁和重量级锁](#64-无锁偏向锁轻量级锁和重量级锁)
    5. [6.5. 公平锁和非公平锁](#65-公平锁和非公平锁)
    6. [6.6. 可重入锁和不可重入锁](#66-可重入锁和不可重入锁)
    7. [6.7. 独享锁和共享锁](#67-独享锁和共享锁)
7. [7. JUC](#7-juc)
    1. [7.1. AQS（AbstractQueuedSynchronizer）](#71-aqsabstractqueuedsynchronizer)
    2. [7.2. FutureTask](#72-futuretask)
    3. [7.3. BlockingQueue](#73-blockingqueue)
    4. [7.4. ForkJoin](#74-forkjoin)
8. [8. 并发编程与扩展](#8-并发编程与扩展)
    1. [8.1. 对高并发的理解](#81-对高并发的理解)
    2. [8.2. 从水平扩展和垂直扩展来提高并发能力](#82-从水平扩展和垂直扩展来提高并发能力)
    3. [8.3. 多线程开发良好的实践](#83-多线程开发良好的实践)
...
## 3. 线程关键字和方法函数相关
### 3.1. wait notify 机制
- 调用 obj 的 wait(), notify() 方法前，必须获得 obj 锁，即必须在同步代码块中使用（synchronized）；
- 调用 obj.wait() 后，线程A就释放了 obj 的锁；
- 当 obj.wait() 方法返回后，线程需要再次获得 obj 锁，才能继续执行；
- obj.notify() 只能唤醒一个进行 obj.wait() 后的线程（具体哪一个由 JVM 决定）；
- obj.notifyAll() 则能全部唤醒，但是要继续执行 obj.wait() 的下一条语句，必须获得 obj 锁，这由优先级和系统调度决定。
...
- 是否一直等待。synchronized 会导致线程拿不到锁一直等待；Lock 可以使用 tryLock() 方法尝试获取锁或者设置获取锁超时时间；
...
### 3.6. join
等待该线程终止。
等待调用 join 方法的线程结束，再继续执行。如：t.join();//主要用于等待t线程运行结束，若无此句，main 则会执行完毕，导致结果不可预测。
...
- workQueue：任务队列
    - ArrayBlockingQueue 基于数组的有界阻塞队列，按 FIFO 排序。线程数最大执行拒绝策略;
...
## 5. 线程安全
多个线程不管以何种方式访问某个类，并且在主调代码中不需要进行同步，都能表现正确行为。
线程安全有以下几种实现方式：
...
### 5.3. 非阻塞同步
互斥同步最主要的问题就是线程阻塞所带来的性能问题，因此这种同步也成为阻塞同步。

**CAS**
乐观锁需要和冲突检测这两个步骤具备原子性，这里就不能再使用互斥同步来保证了，只能依靠硬件来完成。硬件支持的原子性操作最典型的是：比较并交换（Compare and Swap，CAS）。
...
### 6.2. 悲观锁和乐观锁
悲观锁：假设每一次拿数据，都有认为会被修改。synchronized 和 ReentrantLock 等独占锁就是悲观锁思想的实现。例如给数据库的行或表上锁。

乐观锁：就是很乐观，每次去拿数据的时候都认为别人不会修改。可以使用版本号机制和 CAS 算法实现。
...
当线程尝试获取锁时，可重入锁先尝试获取并更新status值，如果 status == 0 表示没有其他线程在执行同步代码，则把 status 置为1，当前线程开始执行。如果 status != 0，则判断当前线程是否是获取到这个锁的线程，如果是的话执行status+1，且当前线程可以再次获取锁。而非可重入锁是直接去获取并尝试更新当前status的值，如果 status != 0 的话会导致其获取锁失败，当前线程阻塞。

释放锁时，可重入锁同样先获取当前status的值，在当前线程是持有锁的线程的前提下。如果 status-1 == 0，则表示当前线程所有重复获取锁的操作都已经执行完毕，然后该线程才会真正释放锁。而非可重入锁则是在确定当前线程是持有锁的线程之后，直接将 status 置为0，将锁释放。
...
## 7. JUC
JUC就是 `java.util .concurrent` 工具包的简称。在此包中增加了并发编程常用工具类,包括线程池,异步IO和轻量级任务框架;还提供了设计用于多线程上下文中的 Collection 实现等。
...
- 多用同步工具少用 wait() 和 notify()。首先，CountDownLatch，CyclicBarrier，Semaphore 和 Exchanger 这些同步类简化了编码操作，而用 wait() 和 notify() 很难实现复杂控制流；其次，这些同步类是- 由最好的企业编写和维护，在后续的 JDK 中还会不断优化和完善。
