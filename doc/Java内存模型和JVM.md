...
- [1. Java内存模型](#1-java内存模型)
	...
	- [1.5. volatile 禁止指令重排序，保证变量对所有线程的可见性](#15-volatile-禁止指令重排序保证变量对所有线程的可见性)
- [2. JVM](#2-jvm)
	- [2.1. JVM 内存模型](#21-jvm-内存模型)
	- [2.2. 内存空间，运行时数据区](#22-内存空间运行时数据区)
		- [2.2.1. 虚拟机栈](#221-虚拟机栈)
...
年轻代分为 Eden 区和 Survivor 区（两块儿：From Survivor  和 To Survivor ），且 Eden:From:To==8:1:1。  
1. 新产生的对象优先分配在 Eden 区（除非配置了`-XX:PretenureSizeThreshold`，大于该值的对象会直接进入年老代）; 
2. 当 Eden 区满了或放不下了，这时候其中存活的对象会复制到 From 区（如果存活下来的对象 From 区都放不下，则这些存活下来的对象全部进入年老代。之后 Eden 区的内存全部回收掉）；
